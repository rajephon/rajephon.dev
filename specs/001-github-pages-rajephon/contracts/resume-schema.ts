...
export function validateMarkdownResumeStructure(htmlContent: string): MarkdownResumeStructure {
  return {
    hasNameHeader: /<h1.*?>.*?<\/h1>/.test(htmlContent),
    hasContactSection: /iconify.*?(charm:person|tabler:mail|tabler:phone)/.test(htmlContent),
...
function extractOptionalSections(htmlContent: string): string[] {
  ...
  sectionMatches.forEach(match => {
    const sectionName = match.replace('## ', '').trim();
