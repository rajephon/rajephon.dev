...
async function generatePDF(language = null) {
  ...
  try {
    ...
    console.log(`Starting PDF generation for languages: ${languages.join(', ')}...`);
    ...
    if (process.platform === 'darwin') {
      ...
      const possiblePaths = [
        ...
        path.join(homeDir, '.cache/puppeteer/chrome/mac-140.0.7339.82/chrome-mac-x64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing'),
      ];
  ...
  generatePDF(targetLanguage)
    .then((results) => {
      ...
      results.forEach(result => {
        console.log(`  📄 ${result.filename} (${result.language.toUpperCase()}) - ${(result.size / 1024).toFixed(2)} KB`);
      });
      process.exit(0);
    })
    .catch((error) => {
