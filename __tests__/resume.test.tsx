...
const MockResumePage: React.FC<MockResumePageProps> = ({ 
  ...
  const [currentLanguage, setCurrentLanguage] = React.useState<'en' | 'ko'>(initialLanguage);
    ...
    <div data-testid="resume-page" lang={currentLanguage}>
      <div data-testid="language-toggle">
        ...
        >
          ...
          {' | '}
          <span className={currentLanguage === 'ko' ? 'text-blue-600' : 'text-gray-600'}>KO</span>
        </button>
      </div>
...
describe('Bilingual Resume Page Integration Tests', () => {
  const mockResumeData: MockResumePageProps['resumeData'] = {
    en: {
      ...
      htmlContent: '<h2>Experience</h2><p>Software Engineer at Example Corp.</p>'
    },
    ko: {
      frontmatter: {
        name: '홍길동',
      ...
      htmlContent: '<h2>경력</h2><p>예시 회사에서 소프트웨어 엔지니어로 근무.</p>'
    }
  };
  ...
  describe('Initial Page Load', () => {
    it('should display English content by default', () => {
      ...
      expect(screen.getByTestId('resume-title')).toHaveTextContent('Software Engineer');
    ...
    it('should respect initialLanguage prop', () => {
      render(<MockResumePage resumeData={mockResumeData} initialLanguage="ko" />);
      
      expect(screen.getByTestId('resume-name')).toHaveTextContent('홍길동');
      expect(screen.getByTestId('resume-title')).toHaveTextContent('소프트웨어 엔지니어');
      expect(screen.getByTestId('resume-summary')).toHaveTextContent('경험 많은 소프트웨어 엔지니어');
    });
  ...
  describe('Content Rendering', () => {
    it('should render HTML content correctly for English', () => {
      ...
      expect(htmlContent.innerHTML).toContain('<p>Software Engineer at Example Corp.</p>');
    });
    ...
    it('should render HTML content correctly for Korean', async () => {
      ...
      await waitFor(() => {
        ...
        expect(htmlContent.innerHTML).toContain('<p>예시 회사에서 소프트웨어 엔지니어로 근무.</p>');
      });
    });
    ...
    it('should display all frontmatter fields correctly', () => {
      ...
      expect(screen.getByTestId('resume-name')).toHaveTextContent('John Doe');
      expect(screen.getByTestId('resume-title')).toHaveTextContent('Software Engineer');
      expect(screen.getByTestId('resume-summary')).toHaveTextContent('Experienced software engineer with 5 years of experience');
    });
  });
  ...
  describe('Accessibility', () => {
    it('should have proper ARIA labels for language toggle', () => {
      ...
      expect(toggleButton).toHaveAttribute('aria-label', 'Toggle language between English and Korean');
    });
