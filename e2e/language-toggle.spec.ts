...
test.describe('Language Toggle E2E Tests', () => {
  ...
  test.describe('User toggles language and downloads PDF', () => {
    ...
    test('should maintain Korean language on page refresh', async ({ page }) => {
      // Switch to Korean
      await page.click('[data-testid="language-toggle"]');
  ...
  test.describe('Language preference persistence across sessions', () => {
    test('should save and restore language preference correctly', async ({ page }) => {
      // Step 1: Navigate to resume page (done in beforeEach)
  ...
  test.describe('Both PDF versions are accessible', () => {
    test('should provide correct PDF links for both languages', async ({ page }) => {
    ...
    test('should have proper MIME types for PDF downloads', async ({ page }) => {
      // Test English PDF
      const englishPdfResponse = await page.request.get('/resume.pdf');
      expect(englishPdfResponse.headers()['content-type']).toContain('application/pdf');
      
      // Test Korean PDF  
      const koreanPdfResponse = await page.request.get('/resume-ko.pdf');
      expect(koreanPdfResponse.headers()['content-type']).toContain('application/pdf');
    });
  });
  ...
  test.describe('Language Toggle UI Behavior', () => {
    test('should highlight current language correctly', async ({ page }) => {
      ...
      const enButton = page.locator('[data-testid="language-toggle"] span').first();
      const koButton = page.locator('[data-testid="language-toggle"] span').last();
    ...
    test('should handle rapid clicks gracefully', async ({ page }) => {
      const toggleButton = page.locator('[data-testid="language-toggle"] button');
  ...
  test.describe('Content Rendering', () => {
    test('should display all sections in both languages', async ({ page }) => {
      ...
      await expect(page.locator('h2')).toContainText(['Experience', 'Education', 'Skills']);
    ...
    test('should maintain layout consistency between languages', async ({ page }) => {
      ...
      expect(Math.abs((initialHeight?.height || 0) - (koreanHeight?.height || 0))).toBeLessThan(100);
    });
  });
